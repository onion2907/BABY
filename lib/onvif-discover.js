...
function extractServices(xml) {
  ...
  for (const block of xml.matchAll(/<[^>]*XAddrs[^>]*>([\s\S]*?)<\/[^>]*XAddrs>/gi)) {
    // XAddrs can list several space-separated service URLs; the device
    // service is the one we want, and any of them carries the host and port.
    for (const url of block[1].trim().split(/\s+/)) {
      const match = url.match(/https?:\/\/([^/\s:]+)/);
      if (!match || match[1] === MULTICAST_ADDR || match[1].startsWith("127.")) continue;
      services.push({ host: match[1], xaddr: url.trim() });
    }
  }
  return services;
}
...
export function discover({ timeoutMs = 4000 } = {}) {
  return new Promise((resolve) => {
    ...
    socket.on("message", (msg) => {
      for (const svc of extractServices(msg.toString())) {
        const existing = byHost.get(svc.host);
        if (!existing || /device_service/i.test(svc.xaddr)) byHost.set(svc.host, svc);
      }
    });
